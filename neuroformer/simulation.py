...
def decode_modality(
    ...
    for it, (x, y) in pbar:
        ...
        if objective == "classification":
            if top_k or top_p != 0:
                logits[modality] = top_k_top_p_filtering(
                    logits[modality], top_k=top_k, top_p=top_p
                )
    ...
    for trial in modality_preds["trial"].unique():
        if prev_trial is None:
            ...
            max_interval = modality_preds[modality_preds["trial"] == prev_trial][
                "interval"
            ].max()
            modality_preds.loc[
                modality_preds["trial"] >= trial, "cum_interval"
            ] += max_interval
