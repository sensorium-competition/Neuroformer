...
class TrainerConfig:
    ...
    score_metrics = [
        ...
        "recall",
        "F1",
    ]  # , 'precision_top5', 'recall_top5', 'F1_top5']
    no_pbar = True
    dist = False
    save_every = 0
    loss_bprop = None
...
class Trainer:
    ...
    def save_checkpoint(self, loss, epoch=None):
        ...
        elif hasattr(raw_model, "tokenizers"):
            with open(os.path.join(self.config.ckpt_path, "tokenizer.pkl"), "wb") as f:
                pickle.dump(raw_model.tokenizers, f)
    ...
    def train(self):
        ...
        def run_epoch(split):
            ...
            for it, (x, y) in pbar:
                ...
                if is_train:
                    ...
                    pbar.set_description(
                        ...
                        + "".join(
                            [
                                f"{str(key)}_{str(split)}: {value:.5f}  "
                                for key, value in loss.items()
                            ]
                        )
                        + f"total_loss: {total_loss.mean():.5f}"
                        + f" lr {lr:e}"
                        + " "
                        + f"precision: {precision.mean():.5f}"
                    )
                    ...
                    if config.use_wandb:
                        ...
                        for score in config.score_metrics:
                            wandb.log(
                                {f"Score/{split}_{str(score)}": preds[score].mean()}
                            )
            ...
            for key, value in losses.items():
                ...
                if config.loss_bprop is not None and key not in config.loss_bprop:
                    continue
                else:
        ...
        for epoch in range(config.max_epochs):
            ...
            if self.config.get_latents:
                self.save_latents(
                    ...
                    self.train_dataset,
                    os.path.join(
                        os.path.dirname(self.config.ckpt_path),
                        "latents",
                        "train",
                        f"epoch_{epoch}",
                    ),
