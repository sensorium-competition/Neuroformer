...
def model_ensemble(models, x):
    ...
    for model in models:
        ...
        logits, _, _ = model(x)
        logits_total["id"] = (
            logits["id"] + logits_total["id"] if "id" in logits_total else logits["id"]
        )
        logits_total["dt"] = (
            logits["dt"] + logits_total["dt"] if "dt" in logits_total else logits["dt"]
        )
    return logits
...
def generate_spikes(
    ...
    def add_sos_eos(x, sos_token=None, eos_token=None, idx_excl=None):
        ...
        x = torch.tensor(
            [sos_token] + x_clean + [eos_token], dtype=torch.long, device=device
        )
        return x, idx_excl
    ...
    device = (
        "cpu" if not gpu else torch.cuda.current_device()
    )  # torch.cuda.current_device() if torch.cuda.is_available() else "cpu"
    model = (
        [model_n.to(device) for model_n in model]
        if isinstance(model, list)
        else model.to(device)
    )
    model = (
        [model_n.eval() for model_n in model]
        if isinstance(model, list)
        else model.eval()
    )
    stoi, itos, itos_dt = (
    ...
    for it, (x, y) in pbar:
        ...
        if true_past is False:
            if it > (window_prev / window):
                df = {
                    ...
                    if k in ("ID", "dt", "Trial", "Interval", "Time")
                }
                df = pd.DataFrame(df)
                ...
                x["dt_prev"] = (
                    torch.tensor(x["dt_prev"][:-1], dtype=x["id"].dtype)
                    .unsqueeze(0)
                    .to(device)
                )
                x["pad_prev"] = (
                    torch.tensor(pad_prev, dtype=x["id_prev"].dtype)
                    .unsqueeze(0)
                    .to(device)
                )
        ...
        for i in range(T_id - 1):  # 1st token is SOS (already there)
            t_pad = torch.tensor(
                [stoi["PAD"]] * (T_id - x["id_full"].shape[-1]), device=device
            )
            t_pad_dt = torch.tensor(
                [0] * (T_id - x["dt_full"].shape[-1]), device=device
            )
            ...
            if top_k or top_p != 0:
                logits["id"] = top_k_top_p_filtering(
                    logits["id"], top_k=top_k, top_p=top_p
                )
                logits["dt"] = top_k_top_p_filtering(
                    logits["dt"], top_k=top_k, top_p=top_p_t
                )
            ...
            if (
                ix.flatten() >= stoi["EOS"]
            ):  # ix >= stoi['EOS']:   # or i > T_id - int(x['pad']):   # or len(current_id_stoi) == T_id: # and dtx == 0.5:    # dtx >= window:   # ix == stoi['EOS']:
            ...
            data["ID"] = (
                data["ID"] + ix_itos.tolist()
            )  # torch.cat((data['ID'], ix_itos))
            data["dt"] = data["dt"] + dtx_itos  # torch.cat((data['dt'], dtx_itos))
            data["Trial"] = (
                data["Trial"] + x["trial"].tolist()
            )  # torch.cat((data['Trial'], x['trial']))
            data["Interval"] = (
                data["Interval"] + x["interval"].tolist()
            )  # torch.cat((data['Interval'], x['interval']))
...
def predict_beam_search(model, loader, stoi, frame_end=0):
    device = (
        "cpu"  # torch.cuda.current_device() if torch.cuda.is_available() else "cpu"
    )
...
def extract_latents(model, dataset):
    ...
    for it, (x, y) in pbar:
        ...
        for modality in features["clip"].keys():
            for idx, (behavior, feat) in enumerate(
                zip(x["behavior"], features["clip"][modality])
            ):
...
def get_class_weights(dataset, stoi, stoi_dt):
    ...
    for x, y in dataset:
        id.extend(
            [stoi["SOS"]] + y["id"][: len(y["id"]) - x["pad"]].flatten().tolist()
        )  # *x['pad']) # -1 in pad to include PAD token
        dt.extend(
            [stoi_dt[0]] + y["dt"][: len(y["dt"]) - x["pad"]].flatten().tolist()
        )  # *x['pad']) # -1 in pad to include PAD token
...
def predict_raster_recursive_time_auto(
    model,
    dataset,
    window,
    window_prev,
