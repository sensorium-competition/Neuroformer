...
from analysis import calc_corr_psth, get_rates_trial
...
def get_id_intervals(df, n_id, intervals):
    ...
    id_intervals[interval_counts.index.astype(int).tolist()] = (
        interval_counts.index.astype(int).tolist()
    )
...
def plot_var(ax, df, variable, values, color_map=None, color=None, m_s=150, l_w=0.5):
    for value in values:
        ...
        ax.scatter(
            data["Time"],
            data[variable],
            color=color,  # c=data[variable].map(color_map),
            marker="|",
            s=m_s,
            linewidth=l_w,
        )
...
def plot_firing_comparison(df_1, df_2, id, trials, intervals, figure_name=None):
    ...
    """
    id_ = id
    true = df_1[(df_1["Trial"].isin(trials)) & (df_1["ID"] == id_)].reset_index(
        drop=True
    )
    pred = df_2[(df_2["Trial"].isin(trials)) & (df_2["ID"] == id_)].reset_index(
        drop=True
    )
    ...
    xticks, xlabels = [0, max_intervals // 2, max_intervals], [
        0,
        max_intervals // 2,
        max_intervals,
    ]
...
def plot_firing_comparison_sweeps(df_1, df_2, id, trials, intervals, figure_name=None):
    ...
    ax_dict_pred = dict()
    for n, trial in enumerate(trials):
        ax_dict_true[trial] = fig.inset_axes(
            [left, bottom * (5 + (n)), width, height_hist]
        )
        ax_dict_pred[trial] = fig.inset_axes(
            [left, bottom * (5 + (len(trials) + n)), width, height_hist],
            sharex=ax_dict_true[trial],
        )
    ...
    for n, trial in enumerate(trials):
        id_ = id
        true = df_1[(df_1["Trial"] == trial) & (df_1["ID"] == id_)].reset_index(
            drop=True
        )
        pred = df_2[(df_2["Trial"] == trial) & (df_2["ID"] == id_)].reset_index(
            drop=True
        )
        if id_ == 345:
            print(true, pred)
        ...
        sns.kdeplot(
            ...
            alpha=0.7,
            warn_singular="False",
        )  # plot(np.array(intervals), rates_1_id, color=colors[0],  lw=3)
        sns.kdeplot(
            true["Interval"],
            ax=ax_hist_1,
            bw_adjust=0.25,
            ...
            alpha=0.7,
            warn_singular="False",
        )  # plot(np.array(intervals), rates_2_id, color=colors[1], lw=3)
...
def set_categorical_ticks(
    ax, yticks=None, ylabels=None, xticks=None, xlabels=None, fs=None
):
...
def plot_neurons_trials_psth(
    df_1, df_2, neurons, trials, intervals, fig_title, figuresize=None
):
    ...
    fig_size = (
        figuresize if figuresize is not None else (2 * scale * len(neurons), 10 * scale)
    )
...
def get_boxplot_data(df_1, df_2, intervals, trials):
    ...
    for n, trial in enumerate(trials):
        ...
        rates_true_prev, rates_true, rates_pred = (
            get_rates_trial(true_prev, intervals),
            get_rates_trial(true, intervals),
            get_rates_trial(pred, intervals),
        )
...
def plot_intertrial_corr(corr_true, corr_pred, trial):
    ...
    def scatter_hist(x, y, ax, ax_histy):
        ...
        ax_hist = sns.distplot(
            y, hist=False, ax=ax_histy, vertical=True
        )  # (x, y, bins=10, orientation='horizontal')
...
def V1_AL_sep_hist(
    atts_V1_AL, atts_V1_AL_rand, corrs_V1_AL, corrs_V1_AL_rand, V1_n, AL_n
):
    set_plot_white()
    ...
    ax[0, 0].errorbar(
        2, V1_att_V1_rand.mean(), yerr=V1_att_V1_rand.std(), c="black", fmt="o"
    )
    ...
    ax[0, 1].errorbar(
        2, AL_att_V1_rand.mean(), yerr=AL_att_V1_rand.std(), c="black", fmt="o"
    )
    ...
    ax[1, 0].errorbar(
        2, V1_corrs_AL_rand.mean(), yerr=V1_corrs_V1_rand.std(), c="black", fmt="o"
    )
    ...
    ax[1, 1].errorbar(
        2, Al_corrs_AL_rand.mean(), yerr=Al_corrs_V1_rand.std(), c="black", fmt="o"
    )
    ...
    score_list = [
        [V1_att_V1, V1_att_AL],
        [AL_att_V1, AL_att_AL],
        [V1_corrs_V1, V1_corrs_AL],
        [AL_corrs_V1, AL_corrs_AL],
    ]
